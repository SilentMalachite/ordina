...
<div class="container mx-auto px-4 py-6">
    <div class="max-w-2xl mx-auto">
        <h1 class="text-3xl font-bold text-gray-800 mb-6">顧客編集</h1>
        ...
        <form method="POST" action="{{ route('customers.update', $customer) }}" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <select name="type" id="type" 
                    ...
                    <option value="company" {{ old('type', $customer->type) == 'company' ? 'selected' : '' }}>法人</option>
                </select>
                @error('type')
            ...
            <div class="mb-4">
                <label for="name" class="block text-gray-700 text-sm font-bold mb-2">
                    <span id="name-label">{{ $customer->type == 'company' ? '会社名' : '名前' }}</span> <span class="text-red-500">*</span>
                </label>
            ...
            <div class="mb-4" id="contact-person-field" style="{{ $customer->type == 'company' ? '' : 'display: none;' }}">
            ...
            <div class="mb-4">
                ...
                <textarea name="address" id="address" rows="3"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('address') border-red-500 @enderror">{{ old('address', $customer->address) }}</textarea>
                @error('address')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="notes" id="notes" rows="3"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('notes') border-red-500 @enderror">{{ old('notes', $customer->notes) }}</textarea>
                @error('notes')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('customers.show', $customer) }}" class="inline-block align-baseline font-bold text-sm text-blue-500 hover:text-blue-800">
                    キャンセル
                </a>
            </div>
        </form>

        <form method="POST" action="{{ route('customers.destroy', $customer) }}" class="text-center">
            @csrf
            @method('DELETE')
            <button type="submit" class="text-red-600 hover:text-red-800 text-sm font-bold"
                onclick="return confirm('本当にこの顧客を削除しますか？関連する取引データがある場合は削除できません。')">
                この顧客を削除
            </button>
        </form>
    </div>
