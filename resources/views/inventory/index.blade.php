...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-800">在庫管理</h1>
        <div class="space-x-2">
            <a href="{{ route('inventory.adjustment.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                在庫調整
            </a>
            <a href="{{ route('inventory.bulk-adjustment') }}" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
                一括調整
            </a>
            <a href="{{ route('inventory.adjustments') }}" class="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                調整履歴
            </a>
        </div>
    </div>
    ...
    @if ($lowStockCount > 0)
        <div class="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded relative mb-4" role="alert">
            <strong>警告:</strong> {{ $lowStockCount }}件の商品が低在庫です。
            <a href="{{ route('inventory.stock-alert') }}" class="underline font-bold">詳細を見る</a>
        </div>
    @endif
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">在庫金額</th>
                    <th class="relative px-6 py-3"><span class="sr-only">操作</span></th>
                </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($products as $product)
                    <tr class="{{ $product->stock_quantity <= 10 ? 'bg-yellow-50' : '' }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            ...
                            <a href="{{ route('inventory.adjustment.create') }}?product_id={{ $product->id }}" class="text-green-600 hover:text-green-900">調整</a>
                        </td>
                    </tr>
                @empty
                    <tr>
